...
function toTrendMode(value) {
    return value === "single" ? "single" : "multi";
}
const PDF_CHINESE_FONT_URL = "https://raw.githubusercontent.com/googlefonts/noto-fonts/main/hinted/ttf/NotoSansTC/NotoSansTC-Regular.ttf";
const state = {
    records: [],
    contentItems: [],
    funnelStages: [],
...
const els = {
    ...
    navButtons: document.querySelectorAll(".app-nav .nav-btn"),
    dashboardSubnavButtons: document.querySelectorAll(".dashboard-subnav-btn"),
    dashboardSubpages: document.querySelectorAll(".dashboard-subpage"),
...
function wireEvents() {
    ...
    els.recordTableBody.addEventListener("change", async (e) => {
        ...
        if (!(target instanceof HTMLInputElement))
            return;
        if (target.dataset.action !== "toggle-note-line")
            return;
        const id = Number(target.dataset.id);
        if (!Number.isFinite(id))
            return;
    ...
    els.contentTableBody.addEventListener("click", async (e) => {
        ...
        if (action === "delete-content") {
            ...
            state.contentItems = await getAllContentItems();
            if (state.contentEditingId === id)
                resetContentFormMode();
            if (state.contentViewingId === id)
...
async function onExportPdfReport() {
    ...
    if (platformSummaries.length) {
        ...
        y += 5;
        const lines = platformSummaries.slice(0, 8).map((item) => `${item.name} 最新 ${item.latest.toLocaleString()}，增長 ${formatSignedPercent(item.growth)}`);
        lines.forEach((line) => {
...
async function onImport(event) {
    ...
    if (hasUnsavedChanges()) {
        const shouldSaveFirst = window.confirm("偵測到你有尚未存檔的變更。按「確定」會先匯出存檔，再載入新檔案。");
        if (shouldSaveFirst) {
    ...
    try {
        ...
        const hasAnyModuleData = Array.isArray(data?.modules?.dashboard?.records) ||
...
async function onClearCache() {
    const ok = confirmTwice("將清除本機緩存與資料（包含紀錄、素材、篩選與離線快取），確定要繼續嗎？", "最後確認：確定清除全部緩存？此操作無法復原。");
    if (!ok)
        return;
    try {
        await clearAllData();
...
function getSelectedStagesForTrend(stages) {
    if (!stages.length)
        return [];
...
function buildPlatformKpiSummaries(snapshots) {
    const stages = getSortedFunnelStages();
    if (!stages.length || !snapshots.length)
        return [];
    const summaries = [];
    for (const stage of stages) {
...
function renderChart(records, snapshots) {
    ...
    const stages = getSortedFunnelStages();
    const snapshotsById = new Map(snapshots.map((x) => [x.id, x]));
    ...
    if (sorted.length === 0 || !selectedStages.length) {
        ...
        if (ctx)
            drawEmptyChartMessage(ctx, els.trendCanvas, "目前區間沒有資料");
        return;
    }
    ...
    const datasets = selectedStages.map((stage, index) => ({
        ...
        data: sorted.map((record) => {
            ...
            if (snap && Number.isFinite(snap.values?.[stage.id]))
                return Number(snap.values?.[stage.id]);
            if (stage.id === dashboardStageIds.firstId)
                return record.threads;
            if (stage.id === dashboardStageIds.secondId && Number.isFinite(record.line))
                return record.line;
            return null;
        }),
    ...
    const markerPlugin = {
        ...
        afterDatasetsDraw(chart) {
            ...
            eventMarkers.forEach((m, i) => {
                if (focused && i !== hoveredMarkerIndex)
                    return;
...
function renderConversionTrendChart(snapshots, records) {
    if (conversionTrendChart) {
...
function filterByRange(records, range) {
    ...
    return records.filter((record) => {
        if (start !== null && record.timestamp < start)
            return false;
        if (end !== null && record.timestamp > end)
            return false;
        return true;
    });
}
function filterFunnelSnapshotsByRange(snapshots, range) {
    const { start, end } = getRangeBounds(range);
    return snapshots
        .filter((snapshot) => {
        if (start !== null && snapshot.timestamp < start)
            return false;
        if (end !== null && snapshot.timestamp > end)
            return false;
        return true;
    })
        .sort((a, b) => a.timestamp - b.timestamp);
...
function clearAllData() {
    return new Promise((resolve, reject) => {
        const txDb = db.transaction([RECORD_STORE_NAME, CONTENT_STORE_NAME, FUNNEL_STAGE_STORE_NAME, FUNNEL_SNAPSHOT_STORE_NAME, KPI_STORE_NAME], "readwrite");
...
function deleteFunnelSnapshot(id) {
    return new Promise((resolve, reject) => {
...
function getAllFunnelSnapshots() {
    return new Promise((resolve, reject) => {
...
function normalizeImportedRecords(rows) {
    return rows
        ...
        if (typeof row.ts === "string") {
            ...
            if (!dt || !Number.isFinite(threads) || threads < 0)
                return null;
            if (line !== null && (!Number.isFinite(line) || line < 0))
                return null;
            return {
                ...
                noteLineEnabled: typeof row.note_line_enabled === "boolean"
        ...
        if (Number.isFinite(row.timestamp) && Number.isFinite(row.threads)) {
            ...
            return {
                ...
                line,
                note: (row.note || "").toString().slice(0, 120),
                noteLineEnabled: typeof row.noteLineEnabled === "boolean"
                    ? row.noteLineEnabled
                    : (row.note || "").toString().trim().length > 0,
                sourceType: "legacy",
...
function extractFunnelStageRowsFromPayload(data) {
    const obj = data;
    if (Array.isArray(obj?.modules?.funnel?.stages))
        return obj.modules.funnel.stages;
    return [];
}
function extractFunnelSnapshotRowsFromPayload(data) {
    const obj = data;
    if (Array.isArray(obj?.modules?.funnel?.snapshots))
        return obj.modules.funnel.snapshots;
    return [];
}
function extractKpiRowsFromPayload(data) {
    const obj = data;
    if (Array.isArray(obj?.modules?.kpi?.items))
        return obj.modules.kpi.items;
    return [];
}
...
function filterFunnelsByKeyword(items, keyword) {
    const q = (keyword || "").trim().toLowerCase();
    if (!q)
        return items;
    const stages = [...state.funnelStages].sort((a, b) => a.order - b.order);
    return items.filter((x) => {
        const stageText = stages
            .map((stage) => `${stage.name}:${Number.isFinite(x.values?.[stage.id]) ? x.values[stage.id] : "-"}`)
            .join(" ");
        const bag = [x.datetime, x.note || "", stageText].join(" ").toLowerCase();
        return bag.includes(q);
    });
}
function filterKpiByKeyword(items, keyword) {
    const q = (keyword || "").trim().toLowerCase();
    if (!q)
        return items;
    return items.filter((x) => {
...
function normalizeImportedFunnelStages(rows) {
    if (!Array.isArray(rows))
        return [];
...
function normalizeImportedFunnelSnapshots(rows, stages) {
    if (!Array.isArray(rows))
        return [];
...
async function buildCanvasReportPages({ roleId, rangeLabel, stageNames, kpi, platformSummaries, records, trendChartDataUrl, conversionChartDataUrl }) {
    try {
        const pages = [];
        ...
        while (remaining.length > 0 || pageIndex === 0) {
            ...
            if (pageIndex === 0) {
                ...
                const platformLines = platformSummaries.slice(0, 6).map((item) => `${item.name} 最新 ${item.latest.toLocaleString()}，增長 ${formatSignedPercent(item.growth)}`);
...
function drawPdfRecordsTable(doc, { records, stageNames, startY, margin, pageW, pageH }) {
...
function drawCanvasRecordsTable(ctx, { x, y, width, records, stageNames, canvasHeight }) {
    const headers = ["時間", stageNames.first, stageNames.second, "備註"];
...
function computeKpiProgress(current, target) {
...
function toKpiMetricKey(value) {
    if (value === "stage1_latest" ||
        value === "stage2_latest" ||
...
function getKpiMetricValue(metricKey, reportKpi, records) {
    ...
    const last = sorted[sorted.length - 1];
    if (metricKey === "stage1_latest")
        return last?.threads ?? NaN;
    if (metricKey === "stage2_latest")
        return Number.isFinite(last?.line) ? Number(last?.line) : NaN;
    if (metricKey === "latest_conversion")
        return reportKpi.latestConversion * 100;
    if (metricKey === "avg_conversion")
        return reportKpi.avgConversion * 100;
    if (metricKey === "stage1_growth")
        return reportKpi.threadsGrowth * 100;
    return reportKpi.lineGrowth * 100;
}
function buildKpiNote(item) {
...
async function onFunnelStageListClick(event) {
    ...
    if (target.dataset.action !== "remove-funnel-stage")
        return;
    const stageId = target.dataset.id || "";
    if (!stageId)
        return;
    ...
    if (!stage)
        return;
    const hasUsed = state.funnelSnapshots.some((x) => Number.isFinite(x.values?.[stageId]));
    if (hasUsed) {
...
function renderContentViewer(items) {
    const selected = items.find((x) => x.id === state.contentViewingId) || (items.length ? items[0] : null);
    if (!selected) {
    ...
    els.contentViewerMeta.textContent = `類型：${selected.type || "-"}｜更新：${selected.updatedAt ? formatDisplayDateTime(new Date(selected.updatedAt)) : "-"}`;
...
function loadSingleTrendStageId() {
    try {
...
function loadMultiTrendStageIds() {
    try {
...
function ensureKpiDefaults(items) {
    return items.map((item) => ({
        ...item,
        metricKey: toKpiMetricKey(item.metricKey || "stage1_latest"),
    }));
}
function persistTrendRangeState(range) {
    localStorage.setItem("ai-kol-trend-range", JSON.stringify(range));
}
function persistConversionRangeState(range) {
...
function persistKpiSearch(keyword) {
...
function persistTrendMode(mode) {
    localStorage.setItem("ai-kol-trend-mode", mode);
}
function persistSingleTrendStageId(stageId) {
    localStorage.setItem("ai-kol-trend-single-stage-id", stageId || "");
}
function persistMultiTrendStageIds(stageIds) {
...
function jumpToRecordFromChart(recordId) {
    state.dashboardTab = "records";
    persistDashboardTab("records");
