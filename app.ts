declare const Chart: {
  ...
  new (canvas: HTMLCanvasElement, config: unknown): TrendChartLike;
};
...
interface Window {
  jspdf?: {
    jsPDF: new (options: unknown) => {
      ...
      addFileToVFS: (name: string, content: string) => void;
...
function toRangeType(value: string): RangeType {
  if (value === "today" || value === "7d" || value === "30d" || value === "custom") return value;
  return "30d";
}

function toSortOrder(value: string): SortOrder {
  return value === "time_asc" ? "time_asc" : "time_desc";
}

function toTrendMode(value: string): TrendMode {
...
interface DashboardRecord {
  ...
  note: string;
  noteLineEnabled: boolean;
  sourceType: "legacy" | "funnel";
  createdAt: string;
  updatedAt: string;
}
...
type DashboardTab = "board" | "records";
...
interface AppState {
  ...
  trendMode: TrendMode;
  singleTrendStageId: string;
  multiTrendStageIds: string[];
  editingId: number | null;
  funnelEditingId: number | null;
  linkModeEnabled: boolean;
  linkModeFromStageId: string;
  contentEditingId: number | null;
  contentViewingId: number | null;
  kpiEditingId: number | null;
}

interface TrendChartLike {
  destroy(): void;
  resize(): void;
  draw(): void;
  resetZoom?: () => void;
  toBase64Image?: (type?: string, quality?: number) => string;
  $hoverEventMarker?: boolean;
}
...
const state: AppState = {
  ...
  kpiSearch: loadKpiSearch(),
  dashboardTab: loadDashboardTab(),
  trendMode: loadTrendMode(),
...
interface ElementsMap {
  ...
  appPages: NodeListOf<HTMLElement>;
  navButtons: NodeListOf<HTMLButtonElement>;
  dashboardSubnavButtons: NodeListOf<HTMLButtonElement>;
  dashboardSubpages: NodeListOf<HTMLElement>;
  routeTriggers: NodeListOf<HTMLElement>;
  recordForm: HTMLFormElement;
  datetimeInput: HTMLInputElement;
  threadsInput: HTMLInputElement;
  lineInput: HTMLInputElement;
  stage1FormLabel: HTMLElement;
  stage2FormLabel: HTMLElement;
  ...
  growthKpiGrid: HTMLElement;
  platformKpiGrid: HTMLElement;
  trendCanvas: HTMLCanvasElement;
  trendModeSelect: HTMLSelectElement;
  trendSingleStageWrap: HTMLElement;
  trendSingleStageSelect: HTMLSelectElement;
  trendStageMultiOptions: HTMLElement;
  conversionTrendCanvas: HTMLCanvasElement;
  showNoteLines: HTMLInputElement;
  exportPdfBtn: HTMLButtonElement;
  resetZoomBtn: HTMLButtonElement;
  resetConversionZoomBtn: HTMLButtonElement;
  sortOrderSelect: HTMLSelectElement;
  roleIdInput: HTMLInputElement;
  lastEditedAt: HTMLElement;
  recordSearchInput: HTMLInputElement;
  clearAllRecordsBtn: HTMLButtonElement;
  recordTableBody: HTMLTableSectionElement;
  recordTableHead: HTMLTableSectionElement;
  rowTemplate: HTMLTemplateElement;
  exportBtn: HTMLButtonElement;
  importInput: HTMLInputElement;
  clearCacheBtn: HTMLButtonElement;
  storageStatus: HTMLElement;
  contentForm: HTMLFormElement;
  contentFormTitle: HTMLElement;
  contentTitleInput: HTMLInputElement;
  contentTypeInput: HTMLInputElement;
  contentTagsInput: HTMLInputElement;
  contentRefInput: HTMLInputElement;
  contentBodyInput: HTMLTextAreaElement;
  saveContentBtn: HTMLButtonElement;
  resetContentBtn: HTMLButtonElement;
  contentTableBody: HTMLTableSectionElement;
  contentRowTemplate: HTMLTemplateElement;
  contentViewerTitle: HTMLElement;
  contentViewerMeta: HTMLElement;
  contentViewerTags: HTMLElement;
  contentViewerRefLink: HTMLAnchorElement;
  contentViewerRefEmpty: HTMLElement;
  contentViewerBody: HTMLElement;
  contentSearchInput: HTMLInputElement;
  clearAllContentBtn: HTMLButtonElement;
  funnelStageNameInput: HTMLInputElement;
  addFunnelStageBtn: HTMLButtonElement;
  toggleLinkModeBtn: HTMLButtonElement;
  funnelZoomOutBtn: HTMLButtonElement;
  funnelZoomInBtn: HTMLButtonElement;
  funnelZoomResetBtn: HTMLButtonElement;
  funnelZoomLabel: HTMLElement;
  linkModeStatus: HTMLElement;
  funnelGraphCanvas: HTMLElement;
  funnelStageList: HTMLElement;
  funnelGraphSvg: SVGSVGElement;
  funnelSnapshotForm: HTMLFormElement;
  funnelDatetimeInput: HTMLInputElement;
  funnelStageInputs: HTMLElement;
  funnelNoteInput: HTMLInputElement;
  saveFunnelSnapshotBtn: HTMLButtonElement;
  resetFunnelSnapshotBtn: HTMLButtonElement;
  funnelSummaryBody: HTMLElement;
  kpiForm: HTMLFormElement;
  kpiNameInput: HTMLInputElement;
  kpiTargetInput: HTMLInputElement;
  kpiMetricSelect: HTMLSelectElement;
  kpiUnitInput: HTMLInputElement;
  kpiDeadlineInput: HTMLInputElement;
  kpiNoteInput: HTMLInputElement;
  saveKpiBtn: HTMLButtonElement;
  resetKpiBtn: HTMLButtonElement;
  kpiSearchInput: HTMLInputElement;
  clearAllKpiBtn: HTMLButtonElement;
  kpiTableBody: HTMLTableSectionElement;
}

function byId<K extends keyof HTMLElementTagNameMap>(id: string, tag: K): HTMLElementTagNameMap[K] {
  const node = document.getElementById(id);
  if (node && node.tagName.toLowerCase() === tag) {
    return node as HTMLElementTagNameMap[K];
  }
  const fallback = document.createElement(tag) as HTMLElementTagNameMap[K];
  fallback.id = id;
...
function bySvgId(id: string): SVGSVGElement {
  const node = document.getElementById(id);
...
const els: ElementsMap = {
  pageTitle: byId("pageTitle", "h1"),
  appPages: document.querySelectorAll<HTMLElement>(".app-page"),
  navButtons: document.querySelectorAll<HTMLButtonElement>(".app-nav .nav-btn"),
  dashboardSubnavButtons: document.querySelectorAll<HTMLButtonElement>(".dashboard-subnav-btn"),
  dashboardSubpages: document.querySelectorAll<HTMLElement>(".dashboard-subpage"),
  routeTriggers: document.querySelectorAll<HTMLElement>("[data-route]"),
  recordForm: byId("recordForm", "form"),
  datetimeInput: byId("datetimeInput", "input"),
  threadsInput: byId("threadsInput", "input"),
  lineInput: byId("lineInput", "input"),
  stage1FormLabel: byId("stage1FormLabel", "span"),
  stage2FormLabel: byId("stage2FormLabel", "span"),
  noteInput: byId("noteInput", "input"),
  ...
  trendSingleStageWrap: byId("trendSingleStageWrap", "label"),
  trendSingleStageSelect: byId("trendSingleStageSelect", "select"),
  trendStageMultiOptions: byId("trendStageMultiOptions", "div"),
  conversionTrendCanvas: byId("conversionTrendCanvas", "canvas"),
  showNoteLines: byId("showNoteLines", "input"),
  exportPdfBtn: byId("exportPdfBtn", "button"),
  resetZoomBtn: byId("resetZoomBtn", "button"),
  resetConversionZoomBtn: byId("resetConversionZoomBtn", "button"),
  sortOrderSelect: byId("sortOrderSelect", "select"),
  roleIdInput: byId("roleIdInput", "input"),
  lastEditedAt: byId("lastEditedAt", "span"),
  ...
  storageStatus: byId("storageStatus", "div"),
  contentForm: byId("contentForm", "form"),
  contentFormTitle: byId("contentFormTitle", "h2"),
  contentTitleInput: byId("contentTitleInput", "input"),
  contentTypeInput: byId("contentTypeInput", "input"),
  contentTagsInput: byId("contentTagsInput", "input"),
  contentRefInput: byId("contentRefInput", "input"),
  contentBodyInput: byId("contentBodyInput", "textarea"),
  saveContentBtn: byId("saveContentBtn", "button"),
  resetContentBtn: byId("resetContentBtn", "button"),
  contentTableBody: byId("contentTableBody", "tbody"),
  contentRowTemplate: byId("contentRowTemplate", "template"),
  contentViewerTitle: byId("contentViewerTitle", "h2"),
  contentViewerMeta: byId("contentViewerMeta", "p"),
  contentViewerTags: byId("contentViewerTags", "p"),
  contentViewerRefLink: byId("contentViewerRefLink", "a"),
  contentViewerRefEmpty: byId("contentViewerRefEmpty", "span"),
  contentViewerBody: byId("contentViewerBody", "article"),
  contentSearchInput: byId("contentSearchInput", "input"),
  ...
  funnelNoteInput: byId("funnelNoteInput", "input"),
  saveFunnelSnapshotBtn: byId("saveFunnelSnapshotBtn", "button"),
  resetFunnelSnapshotBtn: byId("resetFunnelSnapshotBtn", "button"),
  funnelSummaryBody: byId("funnelSummaryBody", "div"),
  kpiForm: byId("kpiForm", "form"),
  kpiNameInput: byId("kpiNameInput", "input"),
  kpiTargetInput: byId("kpiTargetInput", "input"),
  kpiMetricSelect: byId("kpiMetricSelect", "select"),
  kpiUnitInput: byId("kpiUnitInput", "input"),
  kpiDeadlineInput: byId("kpiDeadlineInput", "input"),
  kpiNoteInput: byId("kpiNoteInput", "input"),
  saveKpiBtn: byId("saveKpiBtn", "button"),
  resetKpiBtn: byId("resetKpiBtn", "button"),
  kpiSearchInput: byId("kpiSearchInput", "input"),
  clearAllKpiBtn: byId("clearAllKpiBtn", "button"),
  kpiTableBody: byId("kpiTableBody", "tbody"),
};

let db: IDBDatabase | null = null;
let trendChart: TrendChartLike | null = null;
let conversionTrendChart: TrendChartLike | null = null;
...
function wireEvents() {
  ...
  els.dashboardSubnavButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const tab = toDashboardTab(button.dataset.dashboardTab || "");
  ...
  els.recordTableBody.addEventListener("click", async (e) => {
    ...
    if (action === "delete") {
      ...
      if (state.funnelEditingId === id) resetFunnelSnapshotForm();
      markDataEdited();
      render();
    }
  });
  ...
  els.contentTableBody.addEventListener("click", (e) => {
    ...
    const row = target.closest("tr[data-content-id]") as HTMLElement | null;
    if (!row) return;
  ...
  els.trendModeSelect.addEventListener("change", () => {
    state.trendMode = toTrendMode(els.trendModeSelect.value);
    persistTrendMode(state.trendMode);
    render();
  });
  els.trendSingleStageSelect.addEventListener("change", () => {
    state.singleTrendStageId = els.trendSingleStageSelect.value || "";
    persistSingleTrendStageId(state.singleTrendStageId);
    render();
  });
  els.trendStageMultiOptions.addEventListener("change", (event) => {
    const target = event.target as HTMLInputElement | null;
    if (!target || target.type !== "checkbox") return;
    const stageId = (target.dataset.stageId || "").trim();
    if (!stageId) return;
    const current = new Set(state.multiTrendStageIds);
    if (target.checked) current.add(stageId);
    else current.delete(stageId);
    state.multiTrendStageIds = Array.from(current);
    persistMultiTrendStageIds(state.multiTrendStageIds);
    render();
  });
  ...
  els.sortOrderSelect.addEventListener("change", () => {
    state.sortOrder = toSortOrder(els.sortOrderSelect.value);
  ...
  els.resetConversionZoomBtn.addEventListener("click", () => {
    if (conversionTrendChart && typeof conversionTrendChart.resetZoom === "function") {
      conversionTrendChart.resetZoom();
    }
  });
}

async function onSaveRecord(event: SubmitEvent): Promise<void> {
  ...
  const dashboardRecords = [...getDashboardRecordsForView()]
    ...
    .map((r) => {
      ...
      return {
        ...
        level_1: r.threads,
        level_2: Number.isFinite(r.line) ? r.line : null,
        stage_1_name: stages.first,
        stage_2_name: stages.second,
        threads: r.threads,
    ...
    modules: {
      ...
      funnel: {
        ...
        snapshots: state.funnelSnapshots
          ...
          .sort((a, b) => a.timestamp - b.timestamp)
          .map((x) => ({
            ts: formatTsWithSpace(new Date(x.timestamp)),
            values: x.values,
            note: x.note || "",
...
async function onExportPdfReport() {
  ...
  y += 5;
  doc.text(`${stageNames.first} 增長率：${formatSignedPercent(kpi.threadsGrowth)}`, margin, y);
  y += 5;
  doc.text(`${stageNames.second} 增長率：${formatSignedPercent(kpi.lineGrowth)}`, margin, y);
  y += 6;
  if (platformSummaries.length) {
    doc.text("平台摘要：", margin, y);
    y += 5;
    const lines = platformSummaries.slice(0, 8).map(
      (item) => `${item.name} 最新 ${item.latest.toLocaleString()}，增長 ${formatSignedPercent(item.growth)}`
    );
    lines.forEach((line) => {
  ...
  if (trendChartDataUrl) {
    doc.text("粉絲增長趨勢圖", margin, y);
    y += 4;
    const chartW = pageW - margin * 2;
    const chartH = 70;
    doc.addImage(trendChartDataUrl, "PNG", margin, y, chartW, chartH);
    y += chartH + 7;
  }

  if (conversionChartDataUrl) {
    if (y + 80 > pageH - 24) {
      doc.addPage();
      y = margin;
    }
    doc.text("整體轉換率趨勢圖", margin, y);
    y += 4;
    const chartW = pageW - margin * 2;
    const chartH = 62;
    doc.addImage(conversionChartDataUrl, "PNG", margin, y, chartW, chartH);
    y += chartH + 7;
  }
...
async function onImport(event: Event): Promise<void> {
  ...
  try {
    ...
    render();
    alert(
      `已匯入：紀錄/快照 ${funnelSnapshots.length} 筆、素材 ${validContent.length} 筆、漏斗平台 ${funnelStages.length} 筆`
    );
  } catch (err) {
...
function getDashboardStages(): { first: string; second: string; firstId: string; secondId: string } {
  const stages = getSortedFunnelStages();
  const byId = new Map(stages.map((x) => [x.id, x] as const));
  const firstStage =
    stages.find((x) => !(x.prevIds || []).length) ||
    stages[0];
  const secondStage =
    (firstStage?.nextIds || [])
      .map((id) => byId.get(id))
      .find((x): x is FunnelStage => !!x) ||
...
function applyDashboardStageLabels(): void {
  const stages = getDashboardStages();
...
function updateKpiMetricOptionLabels(stageNames: { first: string; second: string }): void {
  const opts = Array.from(els.kpiMetricSelect.options);
  const setLabel = (value: string, label: string) => {
    const opt = opts.find((x) => x.value === value);
...
function renderTrendStageControls(): void {
  const stages = getSortedFunnelStages();
  state.trendMode = toTrendMode(state.trendMode);
  const validIds = new Set(stages.map((x) => x.id));

  if (!validIds.has(state.singleTrendStageId)) {
    state.singleTrendStageId = stages[0]?.id || "";
    persistSingleTrendStageId(state.singleTrendStageId);
  }

  const filteredMulti = state.multiTrendStageIds.filter((id) => validIds.has(id));
  if (!filteredMulti.length && stages.length) {
    state.multiTrendStageIds = stages.slice(0, Math.min(3, stages.length)).map((x) => x.id);
    persistMultiTrendStageIds(state.multiTrendStageIds);
  } else if (filteredMulti.length !== state.multiTrendStageIds.length) {
    state.multiTrendStageIds = filteredMulti;
    persistMultiTrendStageIds(state.multiTrendStageIds);
  }

  els.trendModeSelect.value = state.trendMode;
  els.trendSingleStageSelect.innerHTML = "";
  for (const stage of stages) {
    const opt = document.createElement("option");
    opt.value = stage.id;
    opt.textContent = stage.name;
    els.trendSingleStageSelect.appendChild(opt);
  }
  if (state.singleTrendStageId) {
    els.trendSingleStageSelect.value = state.singleTrendStageId;
  }

  els.trendStageMultiOptions.innerHTML = "";
  for (const stage of stages) {
    const label = document.createElement("label");
    const input = document.createElement("input");
    input.type = "checkbox";
    input.dataset.stageId = stage.id;
    input.checked = state.multiTrendStageIds.includes(stage.id);
    const text = document.createTextNode(stage.name);
    label.appendChild(input);
    label.appendChild(text);
    els.trendStageMultiOptions.appendChild(label);
  }

  const showSingle = state.trendMode === "single";
  els.trendSingleStageWrap.style.display = showSingle ? "" : "none";
  els.trendStageMultiOptions.style.display = showSingle ? "none" : "";
}

function getSelectedStagesForTrend(stages: FunnelStage[]): FunnelStage[] {
  if (!stages.length) return [];
  if (state.trendMode === "single") {
    const single = stages.find((x) => x.id === state.singleTrendStageId) || stages[0];
    return single ? [single] : [];
  }
  const picked = stages.filter((x) => state.multiTrendStageIds.includes(x.id));
  return picked.length ? picked : stages.slice(0, Math.min(3, stages.length));
}


function goToPage(route: string): void {
  const normalized = normalizeRoute(route);
  const targetHash = normalized === "dashboard" ? `#dashboard-${state.dashboardTab}` : `#${normalized}`;
...
function syncRouteFromHash() {
  ...
  if (raw.startsWith("dashboard-")) {
    state.dashboardTab = toDashboardTab(raw.slice("dashboard-".length));
    persistDashboardTab(state.dashboardTab);
  }
  const route = normalizeRoute(raw);
  setActivePage(route);
}

function normalizeRoute(route: string): "home" | "dashboard" | "content" {
...
function applyDashboardTabToUi(): void {
  els.dashboardSubnavButtons.forEach((button) => {
    const tab = toDashboardTab(button.dataset.dashboardTab || "");
    button.classList.toggle("active", tab === state.dashboardTab);
  });
  els.dashboardSubpages.forEach((section) => {
    const tab = toDashboardTab(section.dataset.dashboardSubpage || "");
    section.classList.toggle("active", tab === state.dashboardTab);
  });
}

function goToFunnelSection(): void {
...
function setActivePage(route: string): void {
  ...
  if (route === "dashboard") {
    els.pageTitle.textContent = "增長儀表板";
    applyDashboardTabToUi();
    render();
    if (trendChart) trendChart.resize();
    if (conversionTrendChart) conversionTrendChart.resize();
    return;
  }
...
function renderTable(records: DashboardRecord[]): void {
  els.recordTableBody.innerHTML = "";
  const stages = getSortedFunnelStages();
  const stageIdSet = new Set(stages.map((s) => s.id));
  const columns = stages.slice();
  const extraStageIds = new Set<string>();
  for (const record of records) {
    for (const stageId of Object.keys(record.stageValues || {})) {
      if (!stageIdSet.has(stageId)) extraStageIds.add(stageId);
    }
  }
...
function renderRecordTableHead(columns: FunnelStage[]): void {
...
function renderGrowthKpiCards(snapshots: FunnelSnapshot[]): void {
...
function renderPlatformKpiCards(snapshots: FunnelSnapshot[]): void {
  ...
  for (const summary of summaries) {
    const card = document.createElement("article");
    card.className = "kpi-card platform-kpi-card";
    card.innerHTML = `
...
function buildPlatformKpiSummaries(snapshots: FunnelSnapshot[]): Array<{ name: string; latest: number; growth: number }> {
  const stages = getSortedFunnelStages();
  if (!stages.length || !snapshots.length) return [];

  const summaries: Array<{ name: string; latest: number; growth: number }> = [];
  for (const stage of stages) {
    const stageValues = snapshots
      .map((snapshot) => snapshot.values?.[stage.id])
      .filter((x) => Number.isFinite(x))
      .map((x) => Number(x));
    if (!stageValues.length) continue;
    const first = stageValues[0];
    const latest = stageValues[stageValues.length - 1];
    summaries.push({
      name: stage.name,
      latest,
      growth: computeGrowthRateWithFloor(first, latest, 1),
    });
  }
  return summaries;
}

function renderChart(records: DashboardRecord[], snapshots: FunnelSnapshot[]): void {
  const sorted = [...records].sort((a, b) => a.timestamp - b.timestamp);
  const dashboardStageIds = getDashboardStages();
  ...
  const stages = getSortedFunnelStages();
  const snapshotsById = new Map(snapshots.map((x) => [x.id, x] as const));
  const selectedStages = getSelectedStagesForTrend(stages);
  const trendModeLabel = state.trendMode === "single" ? "單平台顯示" : "多平台比較";

  if (sorted.length === 0 || !selectedStages.length) {
    const ctx = els.trendCanvas.getContext("2d");
    if (ctx) drawEmptyChartMessage(ctx, els.trendCanvas, "目前區間沒有資料");
    return;
  }

  const palette = ["#64a7ff", "#ff3c7d", "#6fdc8c", "#f5c35b", "#ac86ff", "#57d0d8", "#ff9966"];
  const datasets = selectedStages.map((stage, index) => ({
    label: stage.name,
    data: sorted.map((record) => {
      const snap = snapshotsById.get(record.id);
      if (snap && Number.isFinite(snap.values?.[stage.id])) return Number(snap.values?.[stage.id]);
      if (stage.id === dashboardStageIds.firstId) return record.threads;
      if (stage.id === dashboardStageIds.secondId && Number.isFinite(record.line)) return record.line;
      return null;
    }),
  ...
  trendChart = new Chart(els.trendCanvas, {
    ...
    options: {
      ...
      onClick(event: unknown, _elements: unknown, chart: any) {
      ...
      plugins: {
        zoom: {
          limits: { x: { minRange: 1 }, y: { minRange: 5 } },
        ...
        tooltip: {
          ...
          callbacks: {
            title(items: any[]) {
              ...
              const modeTitle = `${trendModeLabel}｜${formatDisplayDateTime(new Date(sorted[index].timestamp))}`;
              return modeTitle;
            },
          },
        },
      ...
      scales: {
        ...
        y: {
          ...
          title: {
            display: true,
            text: selectedStages.length === 1 ? `${selectedStages[0].name} 人數` : "平台人數",
            color: "#cfd8f7",
          },
          grid: { color: "rgba(65, 75, 104, 0.34)" },
          ticks: {
...
function renderConversionTrendChart(snapshots: FunnelSnapshot[], records: DashboardRecord[]): void {
  if (conversionTrendChart) {
    conversionTrendChart.destroy();
    conversionTrendChart = null;
  }

  const ctx = els.conversionTrendCanvas.getContext("2d");
  if (!ctx) return;
  ...
  const datasets: Array<Record<string, unknown>> = relationPairs.map((pair, idx) => ({
  ...
  const hasAnyValidPoint = datasets.some((dataset) => {
    const points = (dataset.data as Array<number | null | undefined>) || [];
  ...
  const barDatasets = bucketSeries.datasets.map((dataset) => ({
    ...dataset,
    borderWidth: 0,
    borderRadius: 8,
    barPercentage: 0.72,
  ...
  conversionTrendChart = new Chart(els.conversionTrendCanvas, {
    type: "bar",
    data: {
      labels: bucketSeries.labels,
      datasets: barDatasets as unknown[],
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
        ...
        tooltip: {
          ...
          callbacks: {
            title(items: any[]) {
              ...
              return bucketSeries.labels[idx] || "";
            },
            label(item: any) {
              const val = item.raw;
              if (!Number.isFinite(val)) return `${item.dataset.label}: --`;
              return `${item.dataset.label}: ${Number(val).toFixed(2)}%`;
            },
          },
        },
      },
...
function getStageRelationPairsForConversion(stages: FunnelStage[]): Array<{ from: FunnelStage; to: FunnelStage }> {
  const byId = new Map(stages.map((x) => [x.id, x] as const));
  const pairs: Array<{ from: FunnelStage; to: FunnelStage }> = [];
...
function getRecordStageValue(record: DashboardRecord, stageId: string): number | null {
  const fromMap = record.stageValues?.[stageId];
  if (Number.isFinite(fromMap)) return Number(fromMap);
  const basic = getDashboardStages();
  if (stageId === basic.firstId && Number.isFinite(record.threads)) return Number(record.threads);
  if (stageId === basic.secondId && Number.isFinite(record.line)) return Number(record.line);
  return null;
}

function drawEmptyChartMessage(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, text: string): void {
...
function buildBucketedConversionBarSeries(
  ...
  for (let i = 0; i < timestamps.length; i += 1) {
    ...
    lastIndexByBucket.set(bucket.key, i);
  }

  const bucketRecordIds = bucketKeys.map((bucketKey) => {
    const idx = lastIndexByBucket.get(bucketKey);
    if (idx == null) return NaN;
    return recordIds[idx];
  });

  const bucketDatasets = datasets.map((dataset) => {
    const src = ((dataset.data as Array<number | null | undefined>) || []).map((v) =>
    ...
    for (let i = 0; i < timestamps.length; i += 1) {
      ...
      } else if (!bucketValueMap.has(dayKey)) {
        bucketValueMap.set(dayKey, null);
      }
    }
    return {
      ...dataset,
      data: bucketKeys.map((dayKey) => bucketValueMap.get(dayKey) ?? null),
    };
  });

  return {
    labels: bucketKeys.map((dayKey) => bucketLabelMap.get(dayKey) || dayKey),
    recordIds: bucketRecordIds,
...
function buildNoteMarkers(sorted: DashboardRecord[]) {
  ...
  return sorted
    .map((record: DashboardRecord, index: number) => ({ record, index }))
    .filter((x: { record: DashboardRecord; index: number }) => (x.record.note || "").trim().length > 0 && x.record.noteLineEnabled !== false)
    .map((x: { record: DashboardRecord; index: number }, idx: number) => {
...
function filterByRange(records: DashboardRecord[], range: RangeState): DashboardRecord[] {
  const { start, end } = getRangeBounds(range);

  return records.filter((record) => {
    if (start !== null && record.timestamp < start) return false;
    if (end !== null && record.timestamp > end) return false;
    return true;
  });
}

function filterFunnelSnapshotsByRange(snapshots: FunnelSnapshot[], range: RangeState): FunnelSnapshot[] {
  const { start, end } = getRangeBounds(range);
  return snapshots
    .filter((snapshot) => {
      if (start !== null && snapshot.timestamp < start) return false;
      if (end !== null && snapshot.timestamp > end) return false;
      return true;
    })
    .sort((a, b) => a.timestamp - b.timestamp);
}

function getRangeBounds(range: RangeState): { start: number | null; end: number | null } {
  const now = new Date();
  ...
  if (range.type === "custom") {
    ...
    if (!Number.isFinite(start as number)) start = null;
    if (!Number.isFinite(end as number)) end = null;
  }
...
function openDb(): Promise<IDBDatabase> {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    req.onupgradeneeded = (event) => {
      ...
      if (!database.objectStoreNames.contains(FUNNEL_STAGE_STORE_NAME)) {
        const store = database.createObjectStore(FUNNEL_STAGE_STORE_NAME, { keyPath: "id" });
        store.createIndex("order", "order", { unique: false });
      }
      if (!database.objectStoreNames.contains(FUNNEL_SNAPSHOT_STORE_NAME)) {
        const store = database.createObjectStore(FUNNEL_SNAPSHOT_STORE_NAME, { keyPath: "id" });
        store.createIndex("timestamp", "timestamp", { unique: false });
      }
      if (!database.objectStoreNames.contains(KPI_STORE_NAME)) {
        const store = database.createObjectStore(KPI_STORE_NAME, { keyPath: "id" });
        store.createIndex("updatedAt", "updatedAt", { unique: false });
      }
    };
...
function putRecord(record: DashboardRecord): Promise<void> {
...
function clearAllData() {
  return new Promise<void>((resolve, reject) => {
    const txDb = db.transaction(
      [RECORD_STORE_NAME, CONTENT_STORE_NAME, FUNNEL_STAGE_STORE_NAME, FUNNEL_SNAPSHOT_STORE_NAME, KPI_STORE_NAME],
      "readwrite"
    );
    txDb.objectStore(RECORD_STORE_NAME).clear();
    txDb.objectStore(CONTENT_STORE_NAME).clear();
    txDb.objectStore(FUNNEL_STAGE_STORE_NAME).clear();
    txDb.objectStore(FUNNEL_SNAPSHOT_STORE_NAME).clear();
    txDb.objectStore(KPI_STORE_NAME).clear();
...
function clearAllFunnelsOnly() {
  return new Promise<void>((resolve, reject) => {
    const txDb = db.transaction([FUNNEL_STAGE_STORE_NAME, FUNNEL_SNAPSHOT_STORE_NAME], "readwrite");
    txDb.objectStore(FUNNEL_STAGE_STORE_NAME).clear();
    txDb.objectStore(FUNNEL_SNAPSHOT_STORE_NAME).clear();
    txDb.oncomplete = () => resolve();
    txDb.onerror = () => reject(txDb.error);
...
function clearAllFunnelSnapshotsOnly() {
  return new Promise<void>((resolve, reject) => {
    const req = tx(FUNNEL_SNAPSHOT_STORE_NAME, "readwrite").clear();
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function clearAllKpiOnly() {
  return new Promise<void>((resolve, reject) => {
    const req = tx(KPI_STORE_NAME, "readwrite").clear();
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function getAllRecords(): Promise<DashboardRecord[]> {
  return new Promise<DashboardRecord[]>((resolve, reject) => {
    const req = tx(RECORD_STORE_NAME, "readonly").getAll();
    req.onsuccess = () => {
      const data = (req.result || []) as DashboardRecord[];
...
function putContentItem(item: ContentItem): Promise<void> {
  return new Promise<void>((resolve, reject) => {
...
function deleteContentItem(id: number): Promise<void> {
  return new Promise<void>((resolve, reject) => {
...
function getAllContentItems(): Promise<ContentItem[]> {
  return new Promise<ContentItem[]>((resolve, reject) => {
    const req = tx(CONTENT_STORE_NAME, "readonly").getAll();
    req.onsuccess = () => resolve((req.result || []) as ContentItem[]);
    req.onerror = () => reject(req.error);
  });
}

function putFunnelStage(item: FunnelStage): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const req = tx(FUNNEL_STAGE_STORE_NAME, "readwrite").put(item);
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function deleteFunnelStage(id: string): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const req = tx(FUNNEL_STAGE_STORE_NAME, "readwrite").delete(id);
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function getAllFunnelStages(): Promise<FunnelStage[]> {
  return new Promise<FunnelStage[]>((resolve, reject) => {
    const req = tx(FUNNEL_STAGE_STORE_NAME, "readonly").getAll();
    req.onsuccess = () => {
      const rows = ensureFunnelStageDefaults((req.result || []) as FunnelStage[]);
...
function ensureFunnelStageDefaults(items: FunnelStage[]): FunnelStage[] {
  return items.map((item, idx) => ({
    ...item,
    layer: Number.isFinite(item.layer) ? Math.max(1, Number(item.layer)) : idx + 1,
    x: Number.isFinite((item as FunnelStage).x) ? Math.max(0, Number((item as FunnelStage).x)) : 24 + ((idx % 4) * 260),
    y: Number.isFinite((item as FunnelStage).y) ? Math.max(0, Number((item as FunnelStage).y)) : 24 + (Math.floor(idx / 4) * 120),
    prevIds: Array.isArray(item.prevIds) ? item.prevIds.filter((x) => !!x) : [],
    nextIds: Array.isArray(item.nextIds) ? item.nextIds.filter((x) => !!x) : [],
  }));
}

function putFunnelSnapshot(item: FunnelSnapshot): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const req = tx(FUNNEL_SNAPSHOT_STORE_NAME, "readwrite").put(item);
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function deleteFunnelSnapshot(id: number): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const req = tx(FUNNEL_SNAPSHOT_STORE_NAME, "readwrite").delete(id);
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function getAllFunnelSnapshots(): Promise<FunnelSnapshot[]> {
  return new Promise<FunnelSnapshot[]>((resolve, reject) => {
    const req = tx(FUNNEL_SNAPSHOT_STORE_NAME, "readonly").getAll();
    req.onsuccess = () => {
      const rows = (req.result || []) as FunnelSnapshot[];
...
function deleteKpiItem(id: number): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const req = tx(KPI_STORE_NAME, "readwrite").delete(id);
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}

function getAllKpiItems(): Promise<KpiItem[]> {
  return new Promise<KpiItem[]>((resolve, reject) => {
    const req = tx(KPI_STORE_NAME, "readonly").getAll();
    req.onsuccess = () => {
      const rows = (req.result || []) as KpiItem[];
...
function formatDisplayWithSeconds(date: Date): string {
  const y = date.getFullYear();
  const mo = String(date.getMonth() + 1).padStart(2, "0");
  const d = String(date.getDate()).padStart(2, "0");
  const h = String(date.getHours()).padStart(2, "0");
...
function formatTsWithSpace(date: Date): string {
  const y = date.getFullYear();
  const mo = String(date.getMonth() + 1).padStart(2, "0");
  const d = String(date.getDate()).padStart(2, "0");
  const h = String(date.getHours()).padStart(2, "0");
...
function formatReadableFileDateTime(date: Date): string {
  const y = date.getFullYear();
  const mo = String(date.getMonth() + 1).padStart(2, "0");
  const d = String(date.getDate()).padStart(2, "0");
  const h = String(date.getHours()).padStart(2, "0");
...
function parseTsWithSpace(ts: string): Date | null {
...
function normalizeImportedRecords(rows: unknown[]): DashboardRecord[] {
  return rows
    .map((rowRaw) => {
      const row = rowRaw as {
        ts?: unknown;
        ...
        line?: unknown;
        note?: unknown;
        note_line_enabled?: unknown;
        timestamp?: unknown;
        id?: unknown;
        datetime?: unknown;
        noteLineEnabled?: unknown;
        createdAt?: unknown;
        updatedAt?: unknown;
      };
      ...
      if (typeof row.ts === "string") {
        ...
        const lv1 = row.level_1 != null ? row.level_1 : row.threads;
        const lv2 = row.level_2 != null ? row.level_2 : row.line;
        const threads = Math.floor(Number(lv1));
        const line = lv2 == null || lv2 === "" ? null : Math.floor(Number(lv2));
      ...
      if (Number.isFinite(row.timestamp) && Number.isFinite(row.threads)) {
        ...
        if (line !== null && (!Number.isFinite(line) || line < 0)) return null;
        return {
          id: Number.isFinite(row.id) ? Number(row.id) : Date.now() + Math.floor(Math.random() * 100000),
          timestamp: ts,
          datetime: (row.datetime || formatDisplayDateTime(new Date(ts))).toString(),
          ...
          updatedAt: (row.updatedAt || new Date().toISOString()).toString(),
        };
      }
...
function extractDashboardRowsFromPayload(data: unknown): unknown[] {
  const obj = data as { modules?: { dashboard?: { records?: unknown[] } }; records?: unknown[] };
  if (Array.isArray(obj?.modules?.dashboard?.records)) {
    return obj.modules.dashboard.records;
  }
...
function extractContentRowsFromPayload(data: unknown): unknown[] {
  const obj = data as { modules?: { content_library?: { items?: unknown[] } } };
  if (Array.isArray(obj?.modules?.content_library?.items)) {
    return obj.modules.content_library.items;
  }
  return [];
}

function extractRoleIdFromPayload(data: unknown): string {
  const raw = (data as { meta?: { role_id?: string } })?.meta?.role_id;
  if (!raw) return "";
  return sanitizeRoleId(raw);
}

function extractFunnelStageRowsFromPayload(data: unknown): unknown[] {
  const obj = data as { modules?: { funnel?: { stages?: unknown[] } } };
  if (Array.isArray(obj?.modules?.funnel?.stages)) return obj.modules.funnel.stages;
  return [];
}

function extractFunnelSnapshotRowsFromPayload(data: unknown): unknown[] {
  const obj = data as { modules?: { funnel?: { snapshots?: unknown[] } } };
  if (Array.isArray(obj?.modules?.funnel?.snapshots)) return obj.modules.funnel.snapshots;
  return [];
}

function extractKpiRowsFromPayload(data: unknown): unknown[] {
  const obj = data as { modules?: { kpi?: { items?: unknown[] } } };
  if (Array.isArray(obj?.modules?.kpi?.items)) return obj.modules.kpi.items;
  return [];
}

function filterRecordsByKeyword(records: DashboardRecord[], keyword: string): DashboardRecord[] {
  ...
  return records.filter((r) => {
    ...
    const bag = [
      r.datetime,
      String(r.threads),
      String(Number.isFinite(r.line) ? r.line : ""),
      r.note || "",
      stageText,
    ]
      .join(" ")
...
function filterContentByKeyword(items: ContentItem[], keyword: string): ContentItem[] {
  const q = (keyword || "").trim().toLowerCase();
  if (!q) return items;
...
function filterFunnelsByKeyword(items: FunnelSnapshot[], keyword: string): FunnelSnapshot[] {
  const q = (keyword || "").trim().toLowerCase();
  if (!q) return items;
  const stages = [...state.funnelStages].sort((a, b) => a.order - b.order);
  return items.filter((x) => {
    const stageText = stages
      .map((stage) => `${stage.name}:${Number.isFinite(x.values?.[stage.id] as number) ? x.values[stage.id] : "-"}`)
      .join(" ");
    const bag = [x.datetime, x.note || "", stageText].join(" ").toLowerCase();
    return bag.includes(q);
  });
}

function filterKpiByKeyword(items: KpiItem[], keyword: string): KpiItem[] {
  const q = (keyword || "").trim().toLowerCase();
  if (!q) return items;
  return items.filter((x) => {
    const bag = [x.name, x.note, x.unit, x.deadline, String(x.target), getKpiMetricLabel(toKpiMetricKey(x.metricKey))]
      .join(" ")
      .toLowerCase();
    return bag.includes(q);
  });
...
function normalizeImportedContentItems(rows: unknown[]): ContentItem[] {
  ...
  return rows
    .map((rowRaw) => {
      ...
      return {
        ...
        createdAt: (row?.created_at || new Date().toISOString()).toString(),
        updatedAt: (row?.updated_at || new Date().toISOString()).toString(),
      };
    })
    .filter(Boolean);
}

function normalizeImportedFunnelStages(rows: unknown[]): FunnelStage[] {
  if (!Array.isArray(rows)) return [];
  const now = new Date().toISOString();
  const parseRelationIds = (raw: unknown): string[] => {
  ...
  return rows
    ...
    .filter((x): x is FunnelStage => !!x)
    .sort((a, b) => a.order - b.order)
    .map((x, idx) => ({ ...x, order: idx }));
}

function normalizeImportedFunnelSnapshots(rows: unknown[], stages: FunnelStage[]): FunnelSnapshot[] {
  if (!Array.isArray(rows)) return [];
  const stageIds = new Set(stages.map((x) => x.id));
  return rows
    .map((rowRaw) => {
      const row = rowRaw as { ts?: unknown; values?: unknown; note?: unknown; note_line_enabled?: unknown };
      const dt = typeof row.ts === "string" ? parseTsWithSpace(row.ts) : null;
      if (!dt) return null;
      const values: Record<string, number | null> = {};
      const valRaw = row.values as Record<string, unknown>;
      for (const key of Object.keys(valRaw || {})) {
        if (!stageIds.has(key)) continue;
        const nRaw = valRaw[key];
        if (nRaw == null || nRaw === "") {
          values[key] = null;
          continue;
        }
      ...
      return {
        ...
        datetime: formatDisplayDateTime(dt),
        values,
        note: (row.note || "").toString().slice(0, 80),
        noteLineEnabled:
          typeof row.note_line_enabled === "boolean"
            ? row.note_line_enabled
            : (row.note || "").toString().trim().length > 0,
  ...
  return rows
    .map((rowRaw) => {
      ...
      return {
        ...
        target,
        metricKey: toKpiMetricKey((row.metric_key || "stage1_latest").toString()),
        unit: (row.unit || "").toString().slice(0, 20),
        deadline: (row.deadline || "").toString().slice(0, 20),
        note: (row.note || "").toString().slice(0, 120),
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
    })
    .filter((x): x is KpiItem => !!x);
}

function safeRatio(num: number | null | undefined, den: number | null | undefined): number {
  if (!Number.isFinite(num) || !Number.isFinite(den) || den <= 0) return NaN;
  return num / den;
}

function computeGrowthRate(start: number | null | undefined, end: number | null | undefined): number {
...
function computeGrowthRateWithFloor(start: number, end: number, floor: number): number {
  if (!Number.isFinite(start) || !Number.isFinite(end)) return NaN;
  const base = Math.max(start, floor);
  return (end - start) / base;
}

function computeKpiForReport(records: DashboardRecord[]) {
  const sorted = [...records].sort((a, b) => a.timestamp - b.timestamp);
...
function buildRangeLabel(records: DashboardRecord[]): string {
...
function getConversionChartDataUrl() {
  if (!els.conversionTrendCanvas) return "";
  if (conversionTrendChart && typeof conversionTrendChart.toBase64Image === "function") {
    return conversionTrendChart.toBase64Image("image/png", 1);
  }
  try {
    return els.conversionTrendCanvas.toDataURL("image/png", 1);
  } catch {
    return "";
  }
}

async function ensurePdfChineseFont(doc: any): Promise<boolean> {
  try {
...
async function buildCanvasReportPages({ roleId, rangeLabel, stageNames, kpi, platformSummaries, records, trendChartDataUrl, conversionChartDataUrl }: { roleId: string; rangeLabel: string; stageNames: { first: string; second: string }; kpi: { latestConversion: number; avgConversion: number; threadsGrowth: number; lineGrowth: number }; platformSummaries: Array<{ name: string; latest: number; growth: number }>; records: DashboardRecord[]; trendChartDataUrl: string; conversionChartDataUrl: string; }): Promise<string[]> {
  try {
    const pages = [];
    const remaining = [...records].reverse();
    const trendChartImg = trendChartDataUrl ? await loadImage(trendChartDataUrl) : null;
    const conversionChartImg = conversionChartDataUrl ? await loadImage(conversionChartDataUrl) : null;
    let pageIndex = 0;
    ...
    while (remaining.length > 0 || pageIndex === 0) {
      ...
      if (pageIndex === 0) {
        ...
        ctx.fillText("重點數據", 52, 252);
        ctx.font = "24px 'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif";
        ctx.fillText(`總紀錄數：${records.length} 筆`, 52, 294);
        ctx.fillText(`平台數：${Math.max(getSortedFunnelStages().length, 2)} 個`, 52, 328);
        ctx.fillText(`最新轉換率：${formatPercent(kpi.latestConversion)}`, 52, 362);
        ctx.fillText(`區間平均轉換率：${formatPercent(kpi.avgConversion)}`, 52, 396);
        ctx.fillText(`${stageNames.first} 增長率：${formatSignedPercent(kpi.threadsGrowth)}`, 52, 430);
        ctx.fillText(`${stageNames.second} 增長率：${formatSignedPercent(kpi.lineGrowth)}`, 52, 464);
        const platformLines = platformSummaries.slice(0, 6).map(
          (item) => `${item.name} 最新 ${item.latest.toLocaleString()}，增長 ${formatSignedPercent(item.growth)}`
        );
        ctx.font = "21px 'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif";
        platformLines.forEach((line, idx) => {
          ctx.fillText(`• ${line}`, 52, 500 + idx * 30);
        });

        y = 500 + platformLines.length * 30 + 24;
        if (trendChartImg) {
          ctx.fillStyle = "#111827";
          ctx.font = "bold 30px 'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif";
          ctx.fillText("粉絲增長趨勢圖", 52, y);
          y += 14;
          const chartX = 52;
          const chartY = y;
          ...
          ctx.drawImage(trendChartImg, chartX, chartY, chartW, chartH);
          y += chartH + 34;
        }
        if (conversionChartImg) {
          ctx.fillStyle = "#111827";
          ctx.font = "bold 30px 'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif";
          ctx.fillText("整體轉換率趨勢圖", 52, y);
          y += 14;
          const chartX = 52;
          const chartY = y;
          const chartW = canvas.width - 104;
          ...
          ctx.strokeStyle = "#cbd5e1";
          ctx.strokeRect(chartX, chartY, chartW, chartH);
          ctx.drawImage(conversionChartImg, chartX, chartY, chartW, chartH);
          y += chartH + 40;
        }
        ctx.fillStyle = "#111827";
        ctx.font = "bold 30px 'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif";
...
function drawPdfRecordsTable(doc: any, { records, stageNames, startY, margin, pageW, pageH }: { records: DashboardRecord[]; stageNames: { first: string; second: string }; startY: number; margin: number; pageW: number; pageH: number; }): void {
  const headers = ["時間", stageNames.first, stageNames.second, "備註"];
  const colWidths = [44, 24, 24, pageW - margin * 2 - 92];
  const rowH = 7;
  let y = startY;
  ...
  for (const record of records as DashboardRecord[]) {
    ...
    for (let i = 0; i < cells.length; i += 1) {
      const text = shortenText(cells[i], i === 0 ? 16 : i === 3 ? 18 : 10);
...
function drawCanvasRecordsTable(ctx: CanvasRenderingContext2D, { x, y, width, records, stageNames, canvasHeight }: { x: number; y: number; width: number; records: DashboardRecord[]; stageNames: { first: string; second: string }; canvasHeight: number; }): number {
  const headers = ["時間", stageNames.first, stageNames.second, "備註"];
  const colRatios = [0.3, 0.14, 0.12, 0.44];
  ...
  rows.forEach((record: DashboardRecord, idx: number) => {
    ...
    for (let i = 0; i < cells.length; i += 1) {
      const maxChars = i === 0 ? 16 : i === 3 ? 18 : 10;
...
function sortRecordsForTable(records: DashboardRecord[], sortOrder: SortOrder): DashboardRecord[] {
...
function buildExportPrefix(roleId: string): string {
  const safe = sanitizeRoleId(roleId);
  return safe ? `${safe}-records` : "records";
}

function buildJsonExportFilename(roleId: string, date: Date): string {
  const safe = sanitizeRoleId(roleId) || "Amy";
  return `${safe}_全平台存檔_${formatReadableFileDateTime(date)}.json`;
}

function buildPdfReportFilename(roleId: string, date: Date): string {
  const safe = sanitizeRoleId(roleId) || "Amy";
...
function computeKpiProgress(current: number, target: number): number {
  if (!Number.isFinite(current) || !Number.isFinite(target) || target <= 0) return NaN;
...
function toKpiMetricKey(value: string): KpiMetricKey {
  if (
    value === "stage1_latest" ||
    value === "stage2_latest" ||
    value === "latest_conversion" ||
  ...
  return "stage1_latest";
}

function getKpiMetricLabel(metricKey: KpiMetricKey, stageNames = getDashboardStages()): string {
  if (metricKey === "stage1_latest") return `主要平台最新人數（${stageNames.first}）`;
  if (metricKey === "stage2_latest") return `次要平台最新人數（${stageNames.second}）`;
  if (metricKey === "latest_conversion") return "最新轉換率";
  if (metricKey === "avg_conversion") return "區間平均轉換率";
  if (metricKey === "stage1_growth") return `主要平台增長率（${stageNames.first}）`;
  return `次要平台增長率（${stageNames.second}）`;
}

function getKpiMetricValue(
  metricKey: KpiMetricKey,
  reportKpi: { latestConversion: number; avgConversion: number; threadsGrowth: number; lineGrowth: number },
  records: DashboardRecord[]
): number {
  const sorted = [...records].sort((a, b) => a.timestamp - b.timestamp);
  const last = sorted[sorted.length - 1];
  if (metricKey === "stage1_latest") return last?.threads ?? NaN;
  if (metricKey === "stage2_latest") return Number.isFinite(last?.line) ? Number(last?.line) : NaN;
  if (metricKey === "latest_conversion") return reportKpi.latestConversion * 100;
  if (metricKey === "avg_conversion") return reportKpi.avgConversion * 100;
  if (metricKey === "stage1_growth") return reportKpi.threadsGrowth * 100;
  return reportKpi.lineGrowth * 100;
}

function buildKpiNote(item: KpiItem): string {
  const source = getKpiMetricLabel(toKpiMetricKey(item.metricKey));
  const note = (item.note || "").trim();
  if (!note) return `來源：${source}`;
  return `${note}｜來源：${source}`;
}
...
async function onSaveContentItem(event: SubmitEvent): Promise<void> {
  event.preventDefault();
...
function startEditContent(id: number): void {
...
function setDefaultFunnelDateTime() {
  els.funnelDatetimeInput.value = formatDateTimeLocalValue(new Date());
}

async function seedDefaultFunnelStages() {
  const now = new Date().toISOString();
  const defaults: FunnelStage[] = [
...
async function onAddFunnelStage() {
  const name = els.funnelStageNameInput.value.trim();
  if (!name) {
    alert("請輸入平台名稱");
    return;
  }
  ...
  await putFunnelStage(stage);
  state.funnelStages = await getAllFunnelStages();
  els.funnelStageNameInput.value = "";
  markDataEdited();
  render();
}

async function onFunnelStageListClick(event: Event) {
  if (Date.now() - lastFunnelDragEndedAt < 160) return;
  const target = event.target;
  if (!(target instanceof HTMLButtonElement)) return;
  if (target.dataset.action === "rename-funnel-stage") {
  ...
  if (target.dataset.action !== "remove-funnel-stage") return;
  const stageId = target.dataset.id || "";
  if (!stageId) return;
  const stage = state.funnelStages.find((x) => x.id === stageId);
  if (!stage) return;
  const hasUsed = state.funnelSnapshots.some((x) => Number.isFinite(x.values?.[stageId] as number));
  if (hasUsed) {
    alert("此平台已存在於漏斗快照中，請先刪除相關快照再移除平台。");
    return;
  }
  await deleteFunnelStage(stageId);
  const refreshed = await getAllFunnelStages();
  for (let i = 0; i < refreshed.length; i += 1) {
    if (refreshed[i].order === i) continue;
    await putFunnelStage({ ...refreshed[i], order: i, updatedAt: new Date().toISOString() });
  }
  state.funnelStages = await getAllFunnelStages();
  markDataEdited();
  render();
...
async function onSaveFunnelSnapshot(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  ...
  for (const stage of state.funnelStages) {
    const input = els.funnelStageInputs.querySelector<HTMLInputElement>(`input[data-stage-id="${stage.id}"]`);
    ...
    if (!Number.isFinite(n) || n < 0) {
      alert(`平台「${stage.name}」數值需為空值或正整數`);
      return;
    }
    values[stage.id] = n;
  }
...
function startEditFunnelSnapshot(id: number): void {
...
async function onSaveKpiItem(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  const name = els.kpiNameInput.value.trim();
  const target = Number(els.kpiTargetInput.value);
  const metricKey = toKpiMetricKey(els.kpiMetricSelect.value);
  if (!name) {
    alert("KPI 名稱不能為空");
    return;
  }
  ...
  const existing = state.kpiEditingId ? state.kpiItems.find((x) => x.id === state.kpiEditingId) : null;
  const item: KpiItem = {
    id: existing ? existing.id : Date.now() + Math.floor(Math.random() * 1000),
    name: name.slice(0, 80),
    target,
    metricKey,
    unit: els.kpiUnitInput.value.trim().slice(0, 20),
    deadline: els.kpiDeadlineInput.value || "",
    note: els.kpiNoteInput.value.trim().slice(0, 120),
    createdAt: existing?.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
...
function startEditKpi(id: number): void {
  const item = state.kpiItems.find((x) => x.id === id);
  if (!item) return;
  state.kpiEditingId = id;
  els.saveKpiBtn.textContent = "更新 KPI";
  els.resetKpiBtn.textContent = "取消編輯";
  els.kpiNameInput.value = item.name || "";
  els.kpiTargetInput.value = String(item.target);
  els.kpiMetricSelect.value = toKpiMetricKey(item.metricKey);
  els.kpiUnitInput.value = item.unit || "";
  els.kpiDeadlineInput.value = item.deadline || "";
  els.kpiNoteInput.value = item.note || "";
  els.kpiNameInput.focus();
}

function resetKpiFormMode() {
  state.kpiEditingId = null;
  els.kpiForm.reset();
  els.kpiMetricSelect.value = "stage1_latest";
  els.saveKpiBtn.textContent = "儲存 KPI";
  els.resetKpiBtn.textContent = "清空";
}

function renderContentTable(items: ContentItem[]): void {
  ...
  for (const item of sorted) {
    const row = els.contentRowTemplate.content.firstElementChild!.cloneNode(true) as HTMLTableRowElement;
    row.classList.add("record-row");
    row.dataset.contentId = String(item.id);
    if (item.id === state.contentViewingId) {
      row.classList.add("is-focused");
    }
    const titleCell = row.querySelector('[data-col="title"]') as HTMLTableCellElement;
    const typeCell = row.querySelector('[data-col="type"]') as HTMLTableCellElement;
    const tagsCell = row.querySelector('[data-col="tags"]') as HTMLTableCellElement;
    const updatedCell = row.querySelector('[data-col="updated"]') as HTMLTableCellElement;
    ...
    const actionCell = row.querySelector(".row-actions") as HTMLTableCellElement;
    actionCell.dataset.label = "操作";
    (row.querySelector('[data-action="view-content"]') as HTMLButtonElement).dataset.id = String(item.id);
    (row.querySelector('[data-action="edit-content"]') as HTMLButtonElement).dataset.id = String(item.id);
    (row.querySelector('[data-action="delete-content"]') as HTMLButtonElement).dataset.id = String(item.id);
...
function renderFunnelStages(): void {
  ...
  for (const stage of sorted) {
    ...
    btn.dataset.action = "remove-funnel-stage";
    btn.dataset.id = stage.id;
    btn.textContent = "移除";
    const renameBtn = document.createElement("button");
    renameBtn.type = "button";
    renameBtn.className = "ghost";
    renameBtn.dataset.action = "rename-funnel-stage";
    renameBtn.dataset.id = stage.id;
    ...
    els.funnelStageList.appendChild(tag);
...
function renderFunnelLinkModeUi(): void {
...
function setFunnelCanvasZoom(next: number): void {
...
function onFunnelCanvasWheel(event: WheelEvent): void {
...
function scheduleDrawFunnelGraphEdges(): void {
  if (isFunnelDrawScheduled) return;
...
function drawFunnelGraphEdges(): void {
  ...
  container.querySelectorAll<HTMLElement>(".funnel-stage-chip").forEach((node) => {
  ...
  const nodeMap = new Map<string, { left: number; top: number; width: number; height: number }>();
  container.querySelectorAll<HTMLElement>(".funnel-stage-chip").forEach((node) => {
...
async function onSelectStageForLink(stageId: string): Promise<void> {
  if (!state.linkModeEnabled) return;
  if (!state.linkModeFromStageId) {
...
async function onFunnelEdgeClick(event: Event): Promise<void> {
  const target = event.target as SVGElement | null;
  const line = target?.closest?.("line[data-from][data-to]") as SVGLineElement | null;
  if (!line) return;
...
async function setStageRelationLink(from: FunnelStage, to: FunnelStage, linked: boolean): Promise<void> {
...
function wouldCreateCycle(startId: string, targetId: string, stages: FunnelStage[]): boolean {
...
function onFunnelStageCanvasMouseDown(event: MouseEvent): void {
  if (event.button !== 0) return;
  const target = event.target as HTMLElement | null;
  if (!target) return;
  if (target.closest("button")) return;
  const chip = target.closest(".funnel-stage-chip") as HTMLElement | null;
  if (!chip) return;
...
function onFunnelStageCanvasMouseMove(event: MouseEvent): void {
  if (!funnelDragState) return;
  const chip = els.funnelStageList.querySelector<HTMLElement>(`.funnel-stage-chip[data-stage-id="${funnelDragState.stageId}"]`);
  if (!chip) return;
...
async function onFunnelStageCanvasMouseUp(): Promise<void> {
  if (!funnelDragState) return;
  const drag = funnelDragState;
  funnelDragState = null;
  const chip = els.funnelStageList.querySelector<HTMLElement>(`.funnel-stage-chip[data-stage-id="${drag.stageId}"]`);
  ...
  if (!stage) return;
  if (stage.x === nextX && stage.y === nextY) return;
  await putFunnelStage({
    ...stage,
    x: nextX,
    y: nextY,
...
function renderFunnelSummary(): void {
  els.funnelSummaryBody.innerHTML = "";
  const stages = [...state.funnelStages].sort((a, b) => a.order - b.order);
  const latest = [...state.funnelSnapshots].sort((a, b) => b.timestamp - a.timestamp)[0];
  if (!stages.length || !latest || stages.length < 2) {
    const p = document.createElement("p");
    p.className = "empty-text";
    p.textContent = "建立至少兩個平台並新增快照後，即可看到漏斗轉換率。";
    els.funnelSummaryBody.appendChild(p);
    return;
  }
  const relationPairs: Array<{ from: FunnelStage; to: FunnelStage }> = [];
  for (const from of stages) {
  ...
  const overallNode = document.createElement("article");
  overallNode.className = "funnel-metric overall";
  overallNode.innerHTML = `<p>整體轉換率</p><h3>${formatPercent(overall)}</h3>`;
  els.funnelSummaryBody.appendChild(overallNode);
}
...
function renderKpiTable(items: KpiItem[]): void {
  ...
  for (const item of sorted) {
    const tr = document.createElement("tr");
    const current = getKpiMetricValue(item.metricKey, reportKpi, dashboardRecords);
    const progress = computeKpiProgress(current, item.target);
    tr.innerHTML = `
      <td data-label="KPI">${escapeHtml(item.name)}</td>
      <td data-label="目標">${formatNumber(item.target)} ${escapeHtml(item.unit || "")}</td>
      <td data-label="目前">${formatNumber(current)} ${escapeHtml(item.unit || "")}</td>
      <td data-label="進度">${formatPercent(progress)}</td>
      <td data-label="期限">${escapeHtml(item.deadline || "-")}</td>
      <td data-label="備註">${escapeHtml(buildKpiNote(item))}</td>
      <td class="row-actions" data-label="操作">
        <button class="ghost" data-action="edit-kpi" data-id="${item.id}">編輯</button>
        <button class="danger" data-action="delete-kpi" data-id="${item.id}">刪除</button>
      </td>
    `;
    els.kpiTableBody.appendChild(tr);
  }
  if (!sorted.length) {
    const tr = document.createElement("tr");
    tr.innerHTML = '<td colspan="7">目前沒有 KPI</td>';
    els.kpiTableBody.appendChild(tr);
  }
}

function renderContentViewer(items: ContentItem[]): void {
  const selected =
...
function loadKpiSearch() {
  try {
    return localStorage.getItem("ai-kol-kpi-search") || "";
  } catch {
    return "";
  }
}
...
function loadDashboardTab(): DashboardTab {
  try {
    return toDashboardTab(localStorage.getItem("ai-kol-dashboard-tab") || "records");
  } catch {
    return "records";
  }
}

function loadTrendMode(): TrendMode {
  try {
    return toTrendMode(localStorage.getItem("ai-kol-trend-mode") || "multi");
  } catch {
    return "multi";
  }
}

function loadSingleTrendStageId(): string {
  try {
    return (localStorage.getItem("ai-kol-trend-single-stage-id") || "").trim();
  } catch {
    return "";
  }
}

function loadMultiTrendStageIds(): string[] {
  try {
    const raw = localStorage.getItem("ai-kol-trend-multi-stage-ids");
    if (!raw) return [];
    const parsed = JSON.parse(raw);
...
function ensureRecordDefaults(records: DashboardRecord[]): DashboardRecord[] {
  return records.map((record) => ({
    ...
    noteLineEnabled:
      typeof record.noteLineEnabled === "boolean"
        ? record.noteLineEnabled
        : (record.note || "").toString().trim().length > 0,
    sourceType: record.sourceType === "funnel" ? "funnel" : "legacy",
  }));
}

function ensureKpiDefaults(items: KpiItem[]): KpiItem[] {
  return items.map((item) => ({
    ...item,
    metricKey: toKpiMetricKey((item as unknown as { metricKey?: string }).metricKey || "stage1_latest"),
  }));
}
...
function persistSortOrder(sortOrder: SortOrder): void {
  localStorage.setItem("ai-kol-sort-order", sortOrder);
}

function persistRoleId(roleId: string): void {
  localStorage.setItem("ai-kol-role-id", sanitizeRoleId(roleId));
}

function persistLastEditedAt(iso: string): void {
  localStorage.setItem("ai-kol-last-edited-at", iso);
}

function persistLastSavedAt(iso: string): void {
  localStorage.setItem("ai-kol-last-saved-at", iso);
}

function persistRecordSearch(keyword: string): void {
  localStorage.setItem("ai-kol-record-search", keyword || "");
}

function persistContentSearch(keyword: string): void {
  localStorage.setItem("ai-kol-content-search", keyword || "");
}

function persistKpiSearch(keyword: string): void {
  localStorage.setItem("ai-kol-kpi-search", keyword || "");
}

function persistDashboardTab(tab: DashboardTab): void {
  localStorage.setItem("ai-kol-dashboard-tab", tab);
}

function persistTrendMode(mode: TrendMode): void {
  localStorage.setItem("ai-kol-trend-mode", mode);
}

function persistSingleTrendStageId(stageId: string): void {
  localStorage.setItem("ai-kol-trend-single-stage-id", stageId || "");
}

function persistMultiTrendStageIds(stageIds: string[]): void {
  localStorage.setItem("ai-kol-trend-multi-stage-ids", JSON.stringify(stageIds || []));
}
...
function confirmTwice(message1: string, message2: string): boolean {
  if (!window.confirm(message1)) return false;
  if (!window.confirm(message2)) return false;
  return true;
}

function computeLastEditedAtFromData(
  records: DashboardRecord[],
  contentItems: ContentItem[],
  funnelStages: FunnelStage[],
  funnelSnapshots: FunnelSnapshot[]
